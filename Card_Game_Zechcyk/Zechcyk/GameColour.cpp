	...
	was_colour_successful(variableReplacedByReference)
{
	...
	playerWithInitiative = player_who_chose_colour;
...
std::vector<short> GameColour::what_player_can_throw(short id_player) {
	...
	if(thrown[playerWithInitiative]->colour != trump && player_cards_in_colour.empty())
...
void  GameColour::add_points_if_meld() {
	if (does_player_meld() == true) {
		if (thrown[playerWithInitiative]->colour == trump){
			players_pointer[playerWithInitiative]->addTrickPoints(NUMBER_OF_POINTS_FOR_MELD_IN_TRUMP_COLOUR);
			note_who_meld(NUMBER_OF_POINTS_FOR_MELD_IN_TRUMP_COLOUR);
		}
		else {
			players_pointer[playerWithInitiative]->addTrickPoints(NUMBER_OF_POINTS_FOR_MELD_IN_OTHER_COLOUR);
...
void GameColour::note_who_meld(short number_of_points) {
	if (playerWithInitiative == player_who_chose_colour)
...
bool GameColour::does_player_meld() {
	...
	if (players_pointer[playerWithInitiative]->checkIfPlayerHaveAKingAndQueenInOneColour() == true) {
		if (players_pointer[playerWithInitiative]->askPlayerIfHeWantToMeld() == true)
			return true;
	}
	else
		return false;
