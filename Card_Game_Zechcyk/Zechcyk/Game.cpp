...
short Game::playerWithInitiative;
...
void Game::setGameLeader(short which_player) {
	playerWithInitiative = which_player;
}
...
short Game::playTrick()
{
	...
	runThrowingCardMechanism(playerWithInitiative);
	whoIsWinningTrick = playerWithInitiative;
	runThrowingCardMechanism(getPlayersTurnsSequence(1));
	whoIsWinningTrick = compareTwoCards(playerWithInitiative, getPlayersTurnsSequence(1));
	...
	playerWithInitiative = whoIsWinningTrick;

	return whoIsWinningTrick;
}

short Game::getPlayersTurnsSequence (short which_after_player_with_initiative) {
	return ((playerWithInitiative + which_after_player_with_initiative) % 3);
}

void Game::runThrowingCardMechanism(short id_player) {
	if (id_player == playerWithInitiative)
		thrown[playerWithInitiative] = players_pointer[playerWithInitiative]->whichCardPlayerThrow();
	else
...
std::vector<short> Game::checkWhichOfHisCardsPlayerCanThrow(short id_player) {
	...
	player_cards_in_colour = players_pointer[id_player]->checkHowManyCardsInThrownCardColourPlayerHave(thrown[playerWithInitiative]->colour);
...
void Game::distribute_game_points() {
	if (variableReplacedByReference == true)
		players_pointer[playerWithInitiative]->addGamePoints(basicStake * 2 ^ (gamePointsMultiplier - 1));
	else {
		players_pointer[(playerWithInitiative + 1) %3]->addGamePoints(basicStake * 2 ^ (gamePointsMultiplier - 1));
		players_pointer[(playerWithInitiative + 2) %3]->addGamePoints(basicStake * 2 ^ (gamePointsMultiplier - 1));
	}
}
