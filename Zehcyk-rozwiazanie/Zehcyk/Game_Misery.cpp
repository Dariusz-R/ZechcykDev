...
Game_Misery::Game_Misery(Player* players_adress, short player_with_initiative_copy)
	: player_who_chose_misery(player_with_initiative_copy)
{
	thrown[0] = NULL;
	thrown[1] = NULL;
	thrown[2] = NULL;
	...
	was_misery_successful = true; 
}



void Game_Misery::play_misery(short game_points_multiplier) {
	...
	if (was_misery_successful == true) {
		players_pointer[which_player_after_player_with_initiative(0)]->add_game_points(BASE_STAKE_FOR_MISERY * 2 ^ (game_points_multiplier - 1));
		cout << "Mizerka wygrana";
		system("pause");
	} else {
		players_pointer[which_player_after_player_with_initiative(1)]->add_game_points(BASE_STAKE_FOR_MISERY * 2 ^ (game_points_multiplier - 1));
		players_pointer[which_player_after_player_with_initiative(2)]->add_game_points(BASE_STAKE_FOR_MISERY * 2 ^ (game_points_multiplier - 1));
		cout << "mizerka przegrana";
		system("pause");
	}
...
void Game_Misery::game_log_actualization(short who, short situation)
{
	string log_line = "";
	short tab_counter = 0;
	log_line.append("\t*");
